...
export const MonitorPage: React.FC = () => {
  ...
  /** Fetch threshold separately */
  const loadThreshold = useCallback(async () => {
    try {
      const thresholdData = await temperatureService.getThreshold(urlDeviceId);
  ...
  const handleUpdateThreshold = useCallback(async (newThreshold: number) => {
    ...
    await loadThreshold(); // 👈 Refresh only threshold
  }, [backendDeviceId, loadThreshold]);
  ...
  }, [isAutoRefresh, fetchData, loadThreshold]);

  /** Initial fetch */
  useEffect(() => {
    fetchData(true);
    loadThreshold();
  }, [fetchData, loadThreshold]);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-red-100 to-orange-100 animate-fadeIn">
        <div className="text-center p-6 bg-white rounded-2xl shadow-xl animate-zoomIn">
          ...
          <button
            onClick={() => { fetchData(true); loadThreshold(); }}
        ...
        <div className="max-w-7xl mx-auto px-6 py-6 flex flex-col md:flex-row justify-between items-center">
          ...
          <div className="mt-4 md:mt-0 flex items-center space-x-4">
            ...
            <RefreshToggle
              ...
              onManualRefresh={() => { fetchData(); loadThreshold(); }}
